//Función para elegir comuna y región
const region_comuna = {
        "regiones": [
        {
...
document.addEventListener("change", function(e) {
  if (e.target.id === "region") {
    ...
    let regionSeleccionada = region_comuna.regiones.find(r => r.nombre === e.target.value);
...
//función para agregar una fecha mayor a la actual por mínimo 3 horas
...
//función para agregar fotos con un máximo de 5
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("submit-btn").addEventListener("click", validateForm);

  const addBtn = document.getElementById("add-photo-btn");
  addBtn.addEventListener("click", () => {
    let cont = document.getElementById("photo-inputs");
    let total = cont.querySelectorAll(".foto-input").length;
    if (total < 5) {
...
document.querySelectorAll("#tabla-adopciones tbody tr").forEach(row => {
      row.addEventListener("click", () => {
        window.location.href = "informacion-adopcion.html";
      });
    });


//Función para agregar a lo más 5 redes sociales
...
function agregarContacto() {
  ...
  if (!red || contadorContactos >= maxContactos) return;
  ...
  div.innerHTML = `
    <label>${red}:</label>
    <input type="text" name="contacto_${contadorContactos}" 
      placeholder="ID o URL de ${red}">
  `;
